...
(function (root) {
  ...
  function buildEvidenceColumns(input) {
    ...
    const evidence = ((input && input.experienceEvidence) || [])
  ...
  function bonusScoreDisplay(score) {
    ...
    if (s.advanceReason === 'gate') {
      ...
      const matchPart = Number.isFinite(match) && match !== Number(s.score)
        ? ('经历匹配 ' + match + ' · ')
        : '';
    ...
    if (applied > 0) return '经历匹配 ' + Number(s.matchScore) + ' · 加分 +' + applied;
