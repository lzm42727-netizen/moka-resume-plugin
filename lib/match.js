...
(function (root) {
  ...
  function toResultView(item) {
    ...
    return {
      ...
      // v3.4.1：带上阶段开始时间，侧栏据此显示「已等待 X」，长请求期间不再像卡死
      ...(item && item.stage && item.stageSince ? { stageSince: item.stageSince } : {}),
  ...
  /**
   * 把人/秒读起来顺口的等待时长文本（v3.4.1）：<60s 用秒，超过用 m+s（如 2m05s）。
   * 纯函数，供结果卡片展示「已等待 X」；非法输入一律回 0s，不抛错。
   */
