...
(function (root) {
  ...
  function isRecommendItem(item) {
    return !!(item && item.score && item.score.score >= 50);
  }

  function isRecommendView(view) {
    return !!(view && view.score && view.score.score >= 50);
  }
  ...
  function summarizeResultViews(views) {
    ...
    list.forEach((v) => {
      ...
      if (v && v.hardPassed === false) hardfail++;
    });
  ...
  function viewMatchesFilter(view, filter) {
    ...
    if (tab === 'hardfail') return !!(view && view.hardPassed === false);
    if (tab === 'error') return !!(view && view.score && view.score.level === '错误');
    if (tab === 'feedback') return !!(view && (view.feedback === 'positive' || view.feedback === 'negative'));
    return true;
  }
  ...
  function dedupeMustHavesAgainstHard(chips, hc) {
    return (chips || []).map((c) => String(c || '').trim()).filter(Boolean)
      .filter((c) => !chipOverlapsHard(c, hc));
  }
