...
(function (root) {
  ...
  const PROMPT_VERSION = 'evidence-first-bonus-cal-v5';
  ...
  function matchScoringPromptBlock(jobType, handwrittenGates, focusKeywords, bonusKeywords) {
    ...
    return [
      '禁止跳步：先根据简历写出 experienceEvidence——只写与岗位职责/重点看直接相关的经历证据，每段相关实习/项目一句话，与岗位无关的经历不要写入——再对照岗位与重点看提炼 highlights，再核对手写门槛，最后才给 matchScore。',
      '【手写硬性门槛】' + (gates.length ? '\n- ' + gates.join('\n- ') : '无'),
      '逐条输出 handwrittenGateResults：按招聘官填写的完整条件核对，简历无证据就判不过；接受同义表达，例如 PS 等同 Photoshop。',
      '语言类硬性门槛（如英语流利、可作为工作语言）优先看简历的实际使用证据：有英文翻译、英文文档/邮件产出、英语环境实习等即视为过；明确写了等级（N1、CET6）的按等级核对，简历有证书或等价表述即过。',
      '【重点看】' + (focus.length ? focus.join('、') : '无'),
      'highlights 只能写与岗位职责/重点看直接对应的亮点，格式为「能力点＋简历对应证据」，例如岗位要内容运营就写「有内容运营实习，负责选题与数据复盘」；与岗位无关的泛泛优点（如细心、沟通好）不要写入 highlights。',
      '【加分看】' + (bonus.length ? bonus.join('、') : '无'),
      '逐条核对加分看并输出 bonusKeywordResults：每条包含 item、met、reason；只有简历提供可核对证据才判 met=true，无证据判 false。',
      '加分看里的语言类条件（英语、英语流利等）：简历出现可核对的语言使用证据（英文翻译经历、英文文档/邮件产出、英语环境实习/沟通）即视为 met=true；不要仅因未写证书编号就判 false。',
