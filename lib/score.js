...
(function (root) {
  ...
  const TIER_BONUS = { nice: 3 };
  /** 必须未满足扣分上限（最多 6 条 × 5） */
  const MUST_PENALTY_CAP = 30;
  /** 重要未满足扣分上限（最多 6 条 × 3） */
  const IMPORTANT_PENALTY_CAP = 18;
  /** @deprecated 旧 nice 字段兼容；新计分统一使用 BONUS_* 常量 */
  const NICE_BONUS_PER = 3;
  const NICE_BONUS_CAP = 15;
  const NICE_MAX_ITEMS = 5;
  const BONUS_POINTS_PER_ITEM = 3;
  const BONUS_POINTS_CAP = 15;
  const BONUS_MAX_ITEMS = 5;
  /** @deprecated 兼容旧引用：等同必须档封顶 */
  const MUST_HAVE_PENALTY_CAP = MUST_PENALTY_CAP;
  ...
  function penaltyForUnmet(unmet) {
    ...
    (unmet || []).forEach((r) => {
      ...
      if (t === 'important') important += TIER_PENALTY.important;
      else if (t === 'must') must += TIER_PENALTY.must;
    });
    must = Math.min(MUST_PENALTY_CAP, must);
    important = Math.min(IMPORTANT_PENALTY_CAP, important);
    return { must, important, total: must + important };
  }

  function bonusForMetNice(mustHaveResults) {
    const metNice = (mustHaveResults || []).filter((r) => (
      r && normalizeMustHaveItem(r.item) && r.met && normalizeTier(r.tier) === 'nice'
    ));
    const counted = metNice.slice(0, NICE_MAX_ITEMS).length;
    return Math.min(NICE_BONUS_CAP, counted * NICE_BONUS_PER);
  }
  ...
  function normalizeModelScoreResponse(raw) {
    ...
    if (!src.dimensions && !hasFiniteScore(src.matchScore)) {
      return scoreErrorResult('模型返回缺少 matchScore');
    }
    return {
  ...
  function renderRequirementChecklist(spec) {
    const s = spec || {};
    const must = arrOf(s.mustHaves, 6);
    const important = arrOf(s.importantHaves, 6);
    const nice = arrOf(s.niceToHaves, NICE_MAX_ITEMS);
    const lines = [];
    if (must.length) lines.push('【必须，未满足每条 −5，上限 −30】\n- ' + must.join('\n- '));
    if (important.length) lines.push('【重要，未满足每条 −3，上限 −18】\n- ' + important.join('\n- '));
    if (nice.length) lines.push('【加分，具备每条 +3，上限 +15；不具备不扣分】\n- ' + nice.join('\n- '));
    return lines.join('\n\n');
  }
