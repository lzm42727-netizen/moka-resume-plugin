...
(function (root) {
  ...
  /** 弹层（模态框 / 下拉菜单 / 浮层）范围：确认按钮只应出现在这些容器里 */
  const MODAL_SELECTOR = '.ant-modal, .ant-modal-wrap, [role="dialog"]';
  ...
  async function findRecommendConfirm() {
    const attempts = [
      { opts: { partial: true, modalOnly: true, actionPanel: false }, ms: 8000 },
      { opts: { partial: true, popupOnly: true, actionPanel: false }, ms: 4000 },
      { texts: [TEXT.recommendConfirm[0]], opts: { partial: true, actionPanel: false }, ms: 4000 }
    ];
  ...
  async function automateRecommend(root) {
    ...
    await sleep(800);
    const confirm = await findRecommendConfirm();
