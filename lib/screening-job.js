...
(function (root) {
  ...
  function formatScreeningProgress(opts) {
    ...
    // name 是刚完成的一位（并发评分时不是「正在评」的那位），措辞避免误导
    if (name) return '已评 ' + count + ' · 刚完成 ' + name + ' · ' + time;
    return '已评 ' + count + ' · ' + time;
  }

  /** 秒为最小单位的耗时文案：48 秒 / 2 分 41 秒 / 1 时 2 分 3 秒 */
  function formatElapsedText(totalSeconds) {
