...
(function (root) {
  ...
  /**
   * 书写形态折叠：全角 → 半角、连续空白折叠、去掉「纯排版用」的空格。
   *
   * 目的：`日语 N1` / `日语N1` / `日语<U+3000>Ｎ1`（全角空格）应当算同一个信号，否则同一条件
   * 会因为打字差异各自 count=1，被「出现 ≥2 次」的门槛滤掉。
   *
   * 折叠范围刻意收窄，只删三类空格：
   *   ① 中文与中文之间（中文本来不用空格断词）
   *   ② 中文与**含数字**的字母数字串之间（`日语 N1` → `日语N1`、`3 年` → `3年`）
   * 中文与纯英文词之间的空格**保留**——`有 Meta 投放经验` 压成 `有Meta投放经验` 反而难读。
   */
  function foldWidthAndSpaces(raw) {
    ...
    t = t.replace(/\s+/g, ' ');
    const CJK = '[' + CJK_CHAR + ']';
    const NUMTOKEN = '[0-9A-Za-z]*[0-9][0-9A-Za-z]*';
    // ① 中文 中文
    t = t.replace(new RegExp('(' + CJK + ') +(?=' + CJK + ')', 'g'), '$1');
    // ② 含数字的字母数字串 ↔ 中文（双向）
    t = t.replace(new RegExp('(' + NUMTOKEN + ') +(?=' + CJK + ')', 'g'), '$1');
    t = t.replace(new RegExp('(' + CJK + ') +(?=' + NUMTOKEN + ')', 'g'), '$1');
    return t.trim();
  }

  /**
   * 展示用归一：只折叠书写形态与引号，不剥任何语义前缀。
   *
   * 用于把同一条件下的多种写法（`未满足：日语 N1` / `缺 日语N1`）显示成一致文案；
   * 与 normalizeCalibrationSignal 的区别是**绝不动极性**——`缺少甲方品牌经验` 原样保留，
   * 不会被剥成 `甲方品牌经验` 那样读起来像优点的样子。
   */
  function normalizeSignalDisplay(raw) {
    const t = foldWidthAndSpaces(stripSignalQuotes(raw));
    return t.slice(0, 40);
  }

  /** 去掉「缺」「未满足」等展示前缀与嵌套引号，得到干净文案 */
  function normalizeMustHaveLabel(raw) {
    let t = foldWidthAndSpaces(raw);
    if (!t) return '';
    let m;
    while ((m = t.match(/[「『"']([^「『」』"']+)[」』"']/))) {
      t = m[1].trim();
    }
    // 长词在前，「缺乏」不可被 `缺` 吞掉（`缺(?!乏)` 与 GAP_PREFIX_RE 同源）
    t = t.replace(/^(缺少|未满足|缺(?!乏))[:：\s]*/g, '').trim();
    t = t.replace(/^缺(?!乏)\s*/, '').trim();
    return t.slice(0, 40);
  }
   ...
   *
   * 次序：① 书写形态折叠（全角/空格）→ ② 缺前缀 → ③ 剥引号 → ④ 否定前缀 → ⑤ 否定后缀。
   * ① 只做形态归一，不触语义，放在最前不会改变任何极性判定结果。
   *
   * @param {string} raw
