...
(function (root) {
  ...
  /** v3.2.0：批量分配端点（v1.6.3 观测确认，全岗位同址、与职位无关）。
   *  真发捕获的模板仍可覆盖；此常量仅供「确认本岗推荐对象」合成默认模板用 */
  const ASSIGN_ENDPOINT_PATH = '/api/outer/ats-pipeline/assignment/update/v2';

  /**
   * v3.2.0：为「确认本岗推荐对象」合成默认模板——新岗位无需真发即可建记录。
   * 字段默认值来自 v1.6.3 的真实观测（resumeType='all'、抄送/阅卷人为空）；
   * 用户真发一次后，真实捕获的模板会以同名覆盖，成为该岗的精确偏好。
   * origin 传页面源以生成绝对地址（不传则为相对路径，页面上下文 fetch 亦可解析）。
   */
  function buildDefaultTemplate(assigneeIds, origin) {
    const ids = sanitizeIdList(assigneeIds, MAX_ASSIGNEES);
    const base = String(origin || '').replace(/\/+$/, '');
    return {
      url: base + ASSIGN_ENDPOINT_PATH,
