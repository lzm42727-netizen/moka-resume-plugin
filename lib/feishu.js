...
(function (root) {
  ...
  function buildScreeningSummaryCard(data) {
    ...
    const topCandidates = Array.isArray(d.topCandidates) ? d.topCandidates.slice(0, 20) : [];
    ...
    if (topCandidates.length > 0) {
      elements.push({
        ...
        text: {
          tag: 'lark_md',
          content: `**🔥 推荐候选人名单（共 ${topCandidates.length} 位 · 50 分以上从高到低）**`
        }
      });
    ...
    const actions = [];
    if (qualifiedCount > 0) {
      const cleanUrl = String(mokaUrl || 'https://app.mokahr.com').split('#')[0];
      const batchActionUrl = `${cleanUrl}#moka_action=batch_recommend&min_score=50`;
  ...
  /**
   * 解析指定的飞书推送目标（群组 / 个人单人群 / 静默不推）
   * @param {string} targetId 选中的目标 ID，例如 'target_xxx'、'__none__' 或空
   * @param {Array} targets 常用目标列表 [{ id, name, webhook }]
   * @param {string} defaultWebhook 兜底 Webhook
   * @returns {{ enabled: boolean, webhook: string|null, name: string }}
   */
  function resolveFeishuTarget(targetId, targets, defaultWebhook) {
    if (targetId === '__none__' || targetId === 'none') {
      return { enabled: false, webhook: null, name: '不推送飞书' };
    }
    const list = Array.isArray(targets) ? targets : [];
    const found = list.find((t) => t && t.id === targetId);
    if (found && found.webhook) {
      return {
        enabled: true,
        webhook: String(found.webhook).trim(),
        name: found.name ? String(found.name).trim() : '指定目标'
      };
    }
    if (defaultWebhook && typeof defaultWebhook === 'string' && defaultWebhook.trim()) {
      return {
        enabled: true,
        webhook: defaultWebhook.trim(),
        name: (found && found.name) || '默认目标'
      };
    }
    return { enabled: false, webhook: null, name: '未配置目标' };
  }
