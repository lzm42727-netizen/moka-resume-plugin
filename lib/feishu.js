...
(function (root) {
  ...
  function buildScreeningSummaryCard(data) {
    ...
    if (qualifiedCount > 0) {
      ...
      const baseUrl = String(mokaUrl || 'https://app.mokahr.com');
      ...
      actions.push({
        ...
        type: 'primary',
        value: { action: 'feishuRecommendByScore', minScore: 50, mokaUrl: baseUrl }
      });
      ...
      // 备用兜底：Bridge 未运行、或走 Webhook 模式（群机器人无法接收卡片回调）时手动用
      actions.push({
        tag: 'button',
        text: { tag: 'plain_text', content: '🌐 备用：新页面执行' },
        type: 'default',
        url: batchActionUrl
      });
    } else {
  ...
  function buildRecommendationResultCard(data) {
    ...
    // 推荐对象（用人部门 / 面试官）：让 HR 在群里就能确认「推给了谁」
