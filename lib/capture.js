...
(function (root) {
  ...
  // 捕获到的请求体可能残留页面上次翻页的游标，重放前必须清掉，否则第一页从中间开始
  const PAGING_FIELDS = ['lastCursor', 'firstCursor', 'cursor', 'offsetInfo'];

  /** 列表接口分页请求体：清掉残留游标，按 lastCursor 原文翻页 */
  function buildSearchPageBody(baseBody, limit, cursor) {
    const out = Object.assign({}, baseBody || {});
    PAGING_FIELDS.forEach((k) => { delete out[k]; });
    if (limit != null) out.limit = limit;
    if (cursor) out.lastCursor = cursor;
    return out;
  }

  /**
   * 让重放的列表请求跟着页面当前职位走。
   * SPA 里换职位后未必再发一次搜索，捕获体里的 jobIds 可能还是上一岗的；
   * 页面上下文知道职位时一律以它为准，否则会拉回上一岗的候选人与 JD。
   */
  function alignSearchBodyToJob(baseBody, searchCtx) {
    const out = Object.assign({}, baseBody || {});
    const ctx = searchCtx || {};
    if (!out.pipelineId && ctx.pipelineId) {
      out.pipelineId = Number(ctx.pipelineId) || ctx.pipelineId;
    }
    const jobIds = Array.isArray(ctx.jobIds) ? ctx.jobIds.filter(Boolean) : [];
    if (jobIds.length) out.jobIds = jobIds.slice();
    return out;
  }
