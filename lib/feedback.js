...
(function (root) {
  ...
  function formatPreferenceLine(entry, kind) {
    ...
    const missing = uniqueGateLabels(s.hardMissing).slice(0, 2).join('、');
    if (missing) bits.push('未过门槛 ' + missing);
    const rawNote = kind === 'recommend' ? (s.highlights || [])[0] : (s.concerns || [])[0];
    const note = rawNote ? normalizeDisplayText(rawNote) : '';
  ...
  /**
   * 展示用文本归一：只折叠全角/空格与引号，不剥语义前缀。
   * 差距文案（`缺少甲方品牌经验`）必须保留否定义，剥成 `甲方品牌经验` 会读成优点。
   */
  function normalizeDisplayText(raw) {
    const cal = calLib();
    if (cal && typeof cal.normalizeSignalDisplay === 'function') {
      return cal.normalizeSignalDisplay(raw);
    }
    return String(raw || '').trim();
  }

  /** 门槛缺项去重：同一条件的多种写法（`未满足：日语 N1` / `缺 日语N1`）只留一份 */
  function uniqueGateLabels(list) {
    const out = [];
    (list || []).forEach((raw) => {
      const t = normalizePrefLabel(raw);
  ...
  /**
   * 反馈文本口径版本：归一化 / 展示逻辑变更时递增，让已评分的旧缓存失效。
   * feedbackRev 是评分缓存 key 的组成部分，不带版本会让新口径与旧缓存混用。
   */
  const FEEDBACK_CONTEXT_VERSION = 'v2';

  function feedbackRevision(record, jobId) {
    const bag = getFeedbackForJob(record, jobId);
    const keys = Object.keys(bag).sort();
    if (!keys.length) return 'none';
    const payload = FEEDBACK_CONTEXT_VERSION + '\n' + keys.map((k) => {
      const e = bag[k];
