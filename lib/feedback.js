...
(function (root) {
  ...
  function normalizePrefLabel(raw) {
    const cal = root.MokaCalibrate || (typeof require !== 'undefined' ? require('./calibrate.js') : null);
  ...
  function buildPreferenceAggregate(record, jobId) {
    ...
    const signals = [];
    countPhrases(missing).filter((x) => x.count >= 2).slice(0, 2).forEach((x) => {
      signals.push('未过门槛「' + x.text + '」×' + x.count);
    });
    countPhrases(concerns).filter((x) => x.count >= 2).slice(0, 2).forEach((x) => {
