...
(function (root) {
  ...
  function screeningItemToRow(item, origin) {
  ...
  function screeningToCsv(items, origin) {
    const rows = [SCREENING_CSV_HEADERS];
    (items || []).forEach((item) => rows.push(screeningItemToRow(item, origin)));
    return toCsv(rows);
  }
